...
public class ImageSpecification {
	...
	public static Specification<Image> listAllByCriteria(List<SearchCriteria> criterias) {
	...
	private static List<Predicate> addPredicates(
			List<SearchCriteria> criterias, Root<Image> root,
	...
	private static void addPredicate(Root<Image> root, CriteriaBuilder builder, 
		...
		if ( criteria.getKey().equalsIgnoreCase("type") ) {
			predicates.add(builder.equal(
					root.<ImageType> get("imageType"), 
					ImageType.valueOf(criteria.getValue().toString())));
			return;
		}
	}
}